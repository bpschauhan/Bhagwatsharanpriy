...
export function GraphNode({ node, active, dimmed, focused, proximity, showLabel, childCount, onSelect, onHover }: GraphNodeProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: dimmed ? 0.48 : 0.84 + proximity * 0.16, scale: active ? 1.08 : focused ? 1.02 + proximity * 0.02 : 1 }}
    ...
    >
      <button
        type="button"
        className={cn(
          "focus-ring-calm relative rounded-lg border text-center shadow-soft outline-none transition-all duration-300 ease-premium",
      ...
      >
        ...
        {active ? (
          <>
            <span className="absolute -inset-2 -z-10 rounded-xl border border-primary/35 bg-primary/10" />
          </>
        ) : null}
      </button>
